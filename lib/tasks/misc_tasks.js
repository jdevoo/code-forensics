...
module.exports = function(taskDef, context, helpers) {
  ...
  return {
    ...
    tasks: function() {
      ...
      taskDef.addTask('webserver', { description: 'Start local http server to display graphs' }, function(done) {
        var script = Path.resolve(Path.join(appConfig.get('basedir'), 'scripts', 'webserver.js'));
        var webserverProcess = child.spawn('node', [script, '-p', appConfig.get('serverPort'), '-d', context.outputDir], {
          stdio: 'inherit'
        });

        webserverProcess.on('exit', function(code) {
          if (code === 0) {
            done();
          } else {
            done(new Error('Webserver exited with code ' + code));
          }
        });

        webserverProcess.on('error', function(err) {
          done(err);
        });
      });
