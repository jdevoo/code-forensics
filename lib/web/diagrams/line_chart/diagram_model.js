...
module.exports = function(configuration, series) {
  ...
  var updateDynamicScales = function() {
    var visibleSeries = _.filter(series, seriesVisible);
    var newScale = diagramSupport.createScale(visibleSeries, actualWidth, actualHeight);
    diagramScale.y = newScale.y;
    plotLine = diagramSupport.createPlotLine(diagramScale);
  };
  ...
  this.onModelChange = function(listener) {
    _.forOwn(visibleGroups, function(groupNamesObservable) {
      groupNamesObservable.subscribe(function() {
        updateDynamicScales();
        listener();
      });
    });
